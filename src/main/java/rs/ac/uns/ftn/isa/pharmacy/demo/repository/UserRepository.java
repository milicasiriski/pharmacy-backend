...
public interface UserRepository extends CrudRepository<User, Long> {
    ...
    @Query(value = "SELECT pharmacy_user.id, email, password, pharmacy.id, enabled, last_password_reset_date, user_type, pharmacy_user.name, surname FROM pharmacy_user JOIN pharmacy_dermatologist ON pharmacy_user.id = pharmacy_dermatologist.dermatologist_id JOIN pharmacy ON pharmacy.id = pharmacy_dermatologist.pharmacy_id WHERE pharmacy.name = :pharmacyName and user_type = 'DERMATOLOGIST'", nativeQuery=true)
