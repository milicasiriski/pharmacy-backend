...
public interface DermatologistRepository extends CrudRepository<Dermatologist, Long> {
    ...
    @Query(value = "SELECT DISTINCT pharmacy_user.id, email, password, pharmacy.id, enabled, last_password_reset_date, user_type, pharmacy_user.name, surname FROM pharmacy_user JOIN dermatologist_employment_mapping ON pharmacy_user.id = dermatologist_employment_mapping.dermatologist_id JOIN pharmacy ON pharmacy.id = dermatologist_employment_mapping.pharmacy_id WHERE pharmacy.name = :pharmacyName and user_type = 'DERMATOLOGIST'", nativeQuery = true)
