...
public interface PharmacyRepository extends CrudRepository<Pharmacy, Long> {
    ...
    @Query(value = "SELECT p.id, about, p.city, p.country, p.latitude, p.longitude, p.street, p.name, p.rating FROM pharmacy AS p JOIN pharmacy_user AS u ON p.id = pharmacy_id where u.id = :pharmacyAdminId AND user_type = 'PHARMACY_ADMIN'", nativeQuery = true)
